...
Epiregulon prunes the network by performing tests of independence on the observed number of cells jointly expressing transcription factor ($TF$), regulatory element ($RE$) and target gene ($TG$) vs the expected number of cells if $TF\cdot{RE}$ and $TG$ are independently expressed. The thresholds at which the features are considered expressed are defined by the `peak_cutoff` and `exp_cutoff` arguments. We implement two tests, the binomial test and the chi-square test. In the binomial test, the expected probability is $P(TF, RE) \cdot P(TG)$, and the number of trials is the total number of cells (or metacells if we set ``), and the observed successes is the number of cells jointly expressing all three elements. In the chi-square test, the expected probability for having all 3 elements active is also $P(TF, RE) \cdot P(TG)$ and the probability otherwise is $1- P(TF, RE) \cdot P(TG)$. The observed cell count for the active category is the number of cells jointly expressing all three elements ($n_{triple}$), and the cell count for the inactive category is $n - n_{triple}$.
...
pruned.regulon <- addLogFC(regulon = pruned.regulon,
                           ...
                           pval.type = "any",
                           logFC_condition = c("Memory CD8+ T", "B"),
                           logFC_ref = "Memory CD4+ T")

pruned.regulon[,c("idxATAC", "target", "tf", "Memory CD8+ T.vs.Memory CD4+ T.logFC", "B.vs.Memory CD4+ T.FDR")]
```

Here we show to use these statistics to filter our regulons.

```{r}
pruned.regulon.filtered <- pruned.regulon[pruned.regulon$"B.vs.Memory CD4+ T.FDR" < 0.05 & pruned.regulon$"B.vs.Memory CD4+ T.logFC" > 0.3, ]
```
