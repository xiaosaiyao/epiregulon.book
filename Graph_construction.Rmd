# Analysis of TF activity and gene regulatory network
...
Sometimes we are interested in knowing what pathways are enriched in the regulon of a particular TF. We can perform gene set enrichment using the enricher function from [clusterProfiler](http://yulab-smu.top/biomedical-knowledge-mining-book/universal-api.html).

Here we first download Hallmark and C2 signatures from MSigDB and then perform gene set enrichment of the known lineage factors. As expected, EBF1 is consistent with a B cell lineage factor, and TBX21 with lymphoid lineage.
...
#plot results
enrichPlot(results = enrichresults, ncol=2, nrow=1)
```

## Differential network analysis {#diffNetwork}

A second approach to investigate differential TF activity is to compare the weights of the target genes between groups. When performing this contrast, it helps to pre-filter the regulons. In this example, we are interested to compare the networks of B cells and CD4 memory T cells. Here we made use the logFC added to the regulons and keep only target genes that show differential expression.

```{r}
regulon.w.filtered <- regulon.w[regulon.w$"B.vs.Memory.CD4..T.FDR" < 0.05 & abs(regulon.w$"B.vs.Memory.CD4..T.logFC") > 0.3, ]

```
Next, we build an edge subtracted graph and calculate the degree centrality of the subtracted graph. We normalize centrality using the default square root function. The top 5 most positive TFs represent lineage factors more active in B cells whereas the bottom 5 TFs present lineage factors enriched in CD4+ memory T cells. We successfully identified the B factors EBF1, PAX5 and IRF4. The putative CD4 memory T cells factors are less understood and remain to be validated.

```{r}
# construct a graph of the CD4.M and NK cells respectively
CD4.M_network <- buildGraph(regulon.w.filtered, weights = "weight", cluster="Memory CD4+ T")
B_network <- buildGraph(regulon.w.filtered, weights = "weight", cluster="B")
...
```
We can visualize the differential network of selected TFs in the clusters of interest.

```{r}
plotDiffNetwork(regulon.w.filtered,
                cutoff = 0,
                tf = c("EBF1"),
                weight = "weight",
